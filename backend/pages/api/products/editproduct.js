...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const imagePath = imageFile ? `${BACKEND_URL}/uploads/${imageFile.newFilename}` : null;
    const videoPath = videoFile ? `${BACKEND_URL}/uploads/${videoFile.newFilename}` : null;
    ...
    try {
      ...
      if (result.modifiedCount === 0) {
        return res.status(404).json({ message: "لطفا مقادیر هر کدام از فیلد های مورد تمایل را تغییر دهید" });
      }
